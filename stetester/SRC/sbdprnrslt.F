SUBROUTINE SBDPRNRSLT( N, M, S, Z, RESULT, INFO, ITEST, ICASE, DUMP, &
...
!  =======                                                                     !
!                                                                              !
!  SBDPRNRSLT prints/checks the results of SBDSVDX SVD tests.                  !
!                                                                              !
!  Arguments:                                                                  !
!  =========                                                                   !
!                                                                              !
...
!           The results of the tests computed in SBDCHKRSLT.                   !
!                                                                              !
!  INFO     (input) INTEGER                                                    !
...
!
!.. Local Array ..
CHARACTER( LEN=8 ), DIMENSION( 5 ) :: STEST  = (/ 'SBDSQR  ', &
                                                  'SBDSDC  ', &
                                                  'SBDSVDXA', &
                                                  'SBDSVDXI', &
                                                  'SBDSVDXV' /)
...
!
     WRITE( UNIT=FUOUT, FMT='(3X,A,1P,4(A,E11.4),A,I5,2(A,E12.4E3))' ) &
            ...
            ', VL=', VLO, ', VU=', VUO
     IF ( DUMP( 4 ) ) WRITE( UNIT=FUDUMP( 4 ), &
                      FMT='(3X,A,1P,4(A,E11.4),A,I5,2(A,E15.7E3))' ) &
                      STEST( ITEST ), ': TIME=', RESULT( 1 ), &
                      ', RESD=', RESULT( 2 ), ', ORTU=', RESULT( 3 ), &
                      ', ORTV=', RESULT( 4 ), ', M=', M, &
                      ', VL=', VLO, ', VU=', VUO
...
IF ( ITEST.GE.3 .AND. DUMP( 8 ) ) THEN
   ...
   ELSE
      WRITE( UNIT=FUDUMP( 5 ), FMT='(A)' ) &
             'S = zeros(NS,1);'
      WRITE( UNIT=FUDUMP( 5 ), FMT='(2(A,I6,A,1P,E15.7E3,A))' ) &
             ( 'S(', J, ')=', S( J ), '; ', J = 1,M ) 
      WRITE( UNIT=FUDUMP( 5 ), FMT='(2(A,I6,A,1P,E15.7E3,A))' ) &
