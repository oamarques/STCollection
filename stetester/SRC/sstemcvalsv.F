...
!  =======                                                                     !
!                                                                              !
!  SSTEMCVALSV deals with the macro that sets intervals for the smallest and   !
...
IF ( IERR /= 0 ) CALL HANDLER( 2, 'VLLIST (subroutine SSTEMCVALSV)' ) 
VLLIST = GETSREAL( MACRO, LIST, NVL )
!
RECORD = GETRECORD( )
LIST => PARSER( RECORD ); NVU = LISTLENGTH( LIST )
ALLOCATE( VULIST( NVU ), STAT=IERR )
IF ( IERR /= 0 ) CALL HANDLER( 2, 'VULIST (subroutine SSTEMCVALSV)' ) 
VULIST = GETSREAL( MACRO, LIST, NVU )
! 
