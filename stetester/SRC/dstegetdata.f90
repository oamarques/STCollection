...
!  =======                                                                     !
!                                                                              !
!  DSTEGETDATA interprets the input file.                                      !
!                                                                              !
!  Arguments:                                                                  !
!  =========                                                                   !
!                                                                              !
...
!
        CALL DSTEMCGLUED( ECOND, EDIST, ESIGN, ISEED, MAXN, NCASE, MACRO, M )
!
   CASE ( 'HBANDA', 'hbanda' )
!
...
!
        CALL DSTEMCMATRIX( MAXN, NCASE, MACRO, LIST, M )
!
   CASE ( 'MATRIXF', 'matrixf' ) 
!
!       Read tridiagonal matrix stored in file.
!
        CALL DSTEMCMATRIXF( MAXN, NCASE, MACRO, LIST, MTRX, M )
!
   CASE ( 'MINTT', 'mintt' ) 
!
...
!
        CALL DSTEMCVALUES( ECOND, EDIST, ESIGN, ISEED, MAXN, NCASE, MACRO, 5, M)
!
   CASE ( 'SNGVALF', 'sngvalf' )
!
!       Read singular value distribution stored in file.
!
        CALL DSTEMCVALUESF( ISEED, MAXN, NCASE, MACRO, LIST, VALS, 7, M )
!
   CASE ( 'SVD', 'svd' ) 
!
!       Computes SVD(B) in the tridiagonal derived from [ 0 B, B' 0 ]. 
...
!
        CALL DSTEMCVALSI( MACRO, ILIU, NILIU )
!
   CASE ( 'VALSV', 'valsv' ) 
!
!       Read lower and upper bounds of intervals to be searched for eigenvalues.
!
        CALL DSTEMCVALSV( MACRO, VLVU, NVLVU )
!
   CASE ( 'VALUES', 'values' ) 
!
!       Built-in eigenvalue distributions.
!
        CALL DSTEMCVALUES( ECOND, EDIST, ESIGN, ISEED, MAXN, NCASE, MACRO, 1, M )
!
   CASE ( 'VALUESF', 'valuesf' ) 
!
!       Read eigenvalue distribution stored in file.
!
        CALL DSTEMCVALUESF( ISEED, MAXN, NCASE, MACRO, LIST, VALS, 3, M )
!
   CASE ( 'END', 'end' ) 
!
!       End of input file.
...
M => FIRST_M
!
IF ( SVD ) WRITE( UNIT=FUOUT, FMT='(A)' ) '** DSTETESTER warning: SVD mode on'
IF ( TGK ) WRITE( UNIT=FUOUT, FMT='(A)' ) '** DSTETESTER warning: TGK mode on'
