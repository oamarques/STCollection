...
!  =======                                                                     !
!                                                                              !
!  DSTEPRNRSLT prints the results of the LAPACK symmetric tridiagonal          !
!  eigensolver tests.                                                          !
!                                                                              !
!  Arguments:                                                                  !
!  =========                                                                   !
...
!           The results of the tests computed in DSTECHKRSLT.                  !
!                                                                              !
!  INFO     (input) INTEGER                                                    !
...
IF ( DUMP( 2 ) ) THEN
   ...
   ELSE IF ( PRESENT( VLO ) .AND. PRESENT( VUO ) ) THEN
           WRITE( UNIT=FUDUMP( 2 ), FMT='(5X,A,1P,2(A,E23.15E3))' ) &
   ...
   IF ( M == 0 ) THEN
      WRITE( UNIT=FUDUMP( 2 ), FMT='(8X,A)' ) &
             'M = 0, W = [ ]'
   ELSE
      WRITE( UNIT=FUDUMP( 2 ), FMT='(8X,A,I5,A,/,(3X,I5,1P,E25.15))' ) &
             'W( 1:', M, ' ) =', ( I, W( I ), I = 1,M )
   END IF
END IF
!
...
IF ( DUMP( 3 ) ) THEN
   ...
   ELSE
      DO I = 1, M
         WRITE( UNIT=FUDUMP( 3 ), FMT='(8X,A,I5,A,1P,/,(3X,E23.15E3))' ) &
                'Z( :,', I, ' ) =', ( Z( J, I ), J = 1,N )
      END DO
   END IF
END IF
...
IF ( DUMP( 4 ) ) THEN
   ...
   ELSE IF ( PRESENT( VLO ) .AND. PRESENT( VUO ) ) THEN
           IF ( INFO == 0 ) THEN
              WRITE( UNIT=FUDUMP( 4 ), FMT='(3X,1P,A,3(A,E11.4),A,I5,2(A,E23.15E3),A,E11.4)' ) &
                     IDTEST( ITEST ), ': TIME=', RESULT( 1 ), &
                     ', RESD=', RESULT( 2 ), ', ORTH=', RESULT( 3 ), &
                     ', M=', M, ', VL= ', VLO, ', VU= ', VUO, &
                     ', GAPMIN=', GAPMINO
           ELSE
              WRITE( UNIT=FUDUMP( 4 ), FMT='(3X,1P,A,3(A,I11),A,I5,2(A,E23.15E3),A)' ) &
                     IDTEST( ITEST ), ': TIME=', -INFO, ', RESD=', FIZZLE, &
...
IF ( DUMP( 7 ) .OR. DUMP( 8 ) ) THEN
   ...
   ELSE
      ...
      WRITE( UNIT=FUDUMP( 5 ), FMT='(2(2(A,I5),A,1P,E23.15E3,A))' ) &
