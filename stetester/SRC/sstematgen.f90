...
!  =======                                                                     !
!                                                                              !
!  SSTEMATGEN generates various kinds of symmetric tridiagonal and             !
!  bidiagonal matrices.                                                        !
!                                                                              !
!  Arguments:                                                                  !
!  =========                                                                   !
...
!
        CALL SSTEDEFVALS( ECOND, EDIST, ESIGN, MTYPE, ISEED, MSIZE, WORK( 1 ) )
!
   CASE ( 2 )
!
!       Built-in tridiagonal matrix.
!
        CALL SSTEDEFTMTRX( MTYPE, MSIZE, D( N+1 ), E( N+1 ) )
!
   CASE ( 3, 7 )
!
...
!
        CALL SSTEDEFBMTRX( MTYPE, MSIZE, D( N+1 ), E( N+1 ), ISEED, WORK )
!
   CASE DEFAULT
!
...
!
           CALL SLATMS( MSIZE, MSIZE, DIST( EDIST ), ISEED, &
                       'N', WORK( IW ), 0, ZERO, ONE, 0, 1, 'N', &
                       WORK( IA ), MSIZE, WORK( IWORK ), INFO )  
           IF ( INFO /= 0 ) CALL HANDLER ( INFO, 'SLATMS' )
           CALL SCOPY( MSIZE, WORK( IA ), MSIZE+1, D( N+1), 1 )
           CALL SCOPY( MSIZE-1, WORK( IA+MSIZE ), MSIZE+1, E( N+1), 1 ) 
!
   END IF
!
   N = N + MSIZE
...
!            3) Map Z into B.
!
           CALL SSQTSHIFT( N, D, E )
           CALL ST_TO_SZ( N, D, E, WORK( IZQ ), WORK( IZE ) )
           CALL SZ_TO_SB( N, WORK( IBA ), WORK( IBB ), WORK( IZQ ), WORK( IZE ) )
!
        ELSE
!
!          Matrix B is already bidiagonal.
!
           CALL SCOPY( N, D, 1, WORK( IBA ), 1 )
           CALL SCOPY( N, E, 1, WORK( IBB ), 1 )
!
        END IF
!
...
! 
        CALL SCOPY( N, D, 1, WORK( IBA ), 1 )
        CALL SCOPY( N, E, 1, WORK( IBB ), 1 )
        CALL SB_TO_ST( N, WORK( IBA ), WORK( IBB ), D, E )
!
END IF
!
E( N ) = ZERO
...
!
IF ( DUMP( 1 ) ) THEN
   WRITE( UNIT=FUDUMP(1), FMT='(5X,A,6X,A,11X,A,/,(I6,1P,2E17.7))' ) &
