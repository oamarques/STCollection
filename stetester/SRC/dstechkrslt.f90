SUBROUTINE DSTECHKRSLT( N, D, E, M, W, Z, LDZ, HBANDR, WORK, RESULT, TASK, INFO )
!
USE DSTEDEFINITIONS
! 
!.. Scalar Arguments ..
...
!      
        ...
        DO I = 1, NCOLS
           ...
           L = L + NDIAG
        END DO
        RNORM = DLANSB( 'M', 'L', NCOLS, NDIAG-1, WORK( N+1 ), NDIAG, WORK )
!
ELSE IF ( M == N ) THEN
!
!       | T - Z*W*Z' | / ( |T|*N*ULP )
...
!
DO I = 0, MB-1 
   CALL DCOPY( NB, Z( 2, I+1 ), 2, WORK( IWRKU+NB*I ), 1 ) ! NB-by-MB (U)
   CALL DCOPY( NB, Z( 1, I+1 ), 2, WORK( IWRKV+NB*I ), 1 ) ! NB-by-MB (V) 
END DO
!
IF ( TASK( 2 ) ) THEN
!
