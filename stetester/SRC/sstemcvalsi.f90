...
!  =======                                                                     !
!                                                                              !
!  SSTEMCVALSI deals with the macro that sets indices of the smallest and      !
...
IF ( IERR /= 0 ) CALL HANDLER( 2, 'ILLIST (subroutine SSTEMCVALSI)' ) 
ILLIST = GETINTGR( MACRO, LIST, NIL )
!
RECORD = GETRECORD( )
LIST => PARSER( RECORD ); NIU = LISTLENGTH( LIST )
ALLOCATE( IULIST( NIU ), STAT=IERR )
IF ( IERR /= 0 ) CALL HANDLER( 2, 'IULIST (subroutine SSTEMCVALSI)' ) 
